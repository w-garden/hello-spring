...
public class AdminBbsController {
	...
	public ModelAndView admin_bbs_write(HttpServletResponse response,HttpSession session,int page)
			throws Exception{
		response.setContentType("text/html;charset=UTF-8");
		PrintWriter out=response.getWriter();
	...
	public ModelAndView  admin_bbs_cont(int no,String state,HttpServletResponse response,HttpServletRequest
        ...
        if(isAdminLogin(session, response)) {//==true : 관리자로 로그인 된 상태
        	...
        	m.addObject("b",bc);//b키이름에 bc객체를 저장
	...
	public ModelAndView admin_bbs_edit_ok(HttpServletRequest request,HttpServletResponse response,
			BbsVO b,HttpSession session) throws Exception{
		response.setContentType("text/html;charset=UTF-8");
		...
		if(isAdminLogin(session, response)) {
			...
			BbsVO db_File=this.adminBbsService.getBbsCont(bbs_no);//DB로 부터 기존 첨부파일명을 구함
			...
			this.adminBbsService.udpateBbs(b);//자료실 수정
	...
	public String admin_bbs_del(HttpServletResponse response,HttpServletRequest request,HttpSession session,
		...
		if(isAdminLogin(session, response)){ //==true가 생략됨.
			...
			BbsVO db_File=this.adminBbsService.getBbsCont(no);//DB로 부터 기본 첨부파일 정보를 가져옴.
			this.adminBbsService.delBbs(no);//번호를 기준으로 DB로 부터 레코드 삭제
